...
facenet_model = load_model(
    "facenet_keras.h5", compile=False
)
...
details["Mukunth"] = {"Address": "123 hazira Gwalior M.P.", "Phone": "8989855234"}
...
class BtpAPP(root):
    ...
    def add_child(self):
        ...
        button1 = ttk.Button(
            window, text="Select the image of the child", command=lambda: select_file1()
        )
        ...
        def save_details():
            ...
            with open("details.txt", "a") as myfile:
                myfile.write(str(d) + " " + file_path)
                myfile.write("\n")

            img = Image.open(file_path)
            image_path = "C:/Users/catch/Desktop/Hackathon/Missing-Child-Identification-BTP-Thesis-master/dataset/train"
            os.mkdir(f"{image_path}/{name}")
            img.save(f"{image_path}/{name}/{name}.jpg")
            window.destroy()
